...
Создание объектов разных типов (`CityBus`, `ExpressBus`), вызов методов базового и дочерних классов
Полиморфизм: вызов одного метода `calculate()` у разных объектов с разным результатом, проверка типов через `isinstance()`
Интеграция с `BusCollection` (ЛР-2): единая коллекция разных наследников, индексация, сортировка, фильтрация по типу через `filter_by_type(...)` 
...
![111](../../img/lab03/1.png)
# Один метод - разное поведение (полиморфизм)
![222](../../img/lab03/2.png)
