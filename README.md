...
## Разные классы работают через один интерфейс `Printable` (`print_all`)
![1111](./img/lab04/1.png)
## Интерфейс как тип: сортировка через `Comparable` + проверки `isinstance`
![2222](./img/lab04/2.png)
## Интеграция с `BusCollection`:
...
- Передавать функции в методы и использовать их как стратегию.
- Применять `map`, `filter`, `sorted`, `lambda` и фабрики функций.
- Реализовать паттерн «Стратегия» через callable-объекты.

## 2. Что добавлено

- `src/lab05/collection.py` — коллекция `FunctionalBusCollection`:
  - `sort_by(key_func)`
  - `filter_by(predicate)`
  - `apply(func)`
- `src/lab05/strategies.py`:
  - стратегии сортировки и фильтрации;
  - фабрика предикатов `make_max_load_filter(...)`;
  - callable-стратегии `DiscountLoadStrategy`, `SpeedRecommendationStrategy`.
- `src/lab05/demo.py` — демонстрация трёх рабочих сценариев.
- `src/lab05/README.md` — подробный отчёт по лабораторной.

## 3. Демка

- Сценарий 1: цепочка `filter -> sort -> apply`.
- Сценарий 2: замена стратегии без изменения кода коллекции.
- Сценарий 3: callable-объекты + `map/filter/lambda`.
...
## ![111111](./img/lab06/1.png)
## ![222222](./img/lab06/2.png)
## ![333333](./img/lab06/3.png)

# ЛР-7 — консольное приложение

CLI для парка автобусов. Запуск: `cd src/lab07 && python3 main.py`

Файлы: `main.py`, `cli.py`, `app.py`, `storage.py`, `exceptions.py`
